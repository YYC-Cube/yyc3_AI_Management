...
**前端**：Next.js 16、React 19、TypeScript 5.x、Tailwind CSS、shadcn/ui、Radix UI、Storybook
**后端**：Node.js 20+、Express.js、TypeScript 5.3+、PostgreSQL 15、Redis 7、Socket.IO、OpenAI SDK
**基础设施**：Docker / Docker Compose、Prometheus + Grafana、GitHub Actions CI/CD
**开发工具**：ESLint、Prettier、Jest（单元/集成）、Playwright（E2E）、K6（性能测试）
...
3. **配置环境变量**
   ...
   ```
   > ⚠️ **重要**: `.env.local` 已包含本地开发默认配置（DB端口5432, 用户postgres, 数据库yyc3_management）
   >
   > 如需自定义，请编辑 `.env.local` 文件
...
5. **访问应用**
   ...
   - 后端API: http://localhost:3139
   - API文档: http://localhost:3139 (HTML页面)
...
### 核心配置项 (.env.local)
- NEXT_PUBLIC_API_BASE_URL=http://localhost:3139
- NEXT_PUBLIC_ENV=development
- NEXT_PUBLIC_WS_URL=ws://localhost:3139
